...
*/
__ALWAYS_INLINE__
static inline int enforce_ingress_policy(__u64 tunnel_id, const struct ipv4_tuple_t *ipv4_tuple, __u32 pod_label_value, __u32 namespace_label_value)
{
	...
	if (pod_label_value > 0) {
		bpf_debug("Checking packet for pod label policy with pod_label_value=%d", pod_label_value);
		...
		if (policies_ppo & policies_pod_label){
			bpf_debug("Packet matches pod label policy with pod_label_value=%d", pod_label_value);
			return 0;
		}		
	}

	if (namespace_label_value > 0) {
		bpf_debug("Checking packet for namespace label policy with namespace_label_value=%d", pod_label_value);
		...
		if (policies_ppo & policies_namespace_label){
			bpf_debug("Packet matches namespace label policy with namespace_label_value=%d", namespace_label_value);
			return 0;
		}
	}

	if (pod_label_value > 0 && namespace_label_value > 0) {
		bpf_debug("Checking packet for pod and namespace label policy with pod_label_value=%d and namespace_label_value=%d", pod_label_value, namespace_label_value);
		...
		if (policies_ppo & policies_pod_and_namespace_label){
			bpf_debug("Packet matches pod label policy with pod_label_value=%d and namespace_label_value=%d", pod_label_value, namespace_label_value);
			return 0;
		}
	}
...
*/
__ALWAYS_INLINE__
static inline int ingress_policy_check(__u64 tunnel_id, const struct ipv4_tuple_t *ipv4_tuple, __u32 pod_label_value, __u32 namespace_label_value)
{
