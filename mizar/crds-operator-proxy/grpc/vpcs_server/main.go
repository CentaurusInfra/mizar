...
// services - ReadVpc
func (s *server) ReadVpc(ctx context.Context, in *pb.VpcId) (*pb.VpcsResponse, error) {
	log.Printf("Received Request - Read: %v", in.GetId())
	return &pb.VpcsResponse{JsonReturncode: "{Vpcs Response: " + in.GetId() + "}"}, nil
}

// services - DeleteVpc
func (s *server) DeleteVpc(ctx context.Context, in *pb.VpcId) (*pb.Empty, error) {
	log.Printf("Received Request - Delete: %v", in.GetId())
	return &pb.Empty{}, nil
}

// services - ResumeVpc
func (s *server) ResumeVpc(ctx context.Context, in *pb.VpcId) (*pb.Empty, error) {
