...
class test_fast_path_switch(unittest.TestCase):
    ...
    def test_fast_path_switch(self):
        logger.info(
            "{} Testing two endpoints on a switch can communicate! {}".format('='*20, '='*20))
        do_validate_fast_path_net(
            self, self.ep_left, self.ep_right, self.droplets["switch"])
